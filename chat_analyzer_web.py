...
<body>
    ...
    <script>
        ...
        // Restore filename from sessionStorage if available (after browser back)
        const savedFileName = sessionStorage.getItem('uploadedFileName');
        if (savedFileName) {
            fileName.textContent = '✓ ' + savedFileName;
        }

        // Also check if file is currently selected
        if (fileInput.files.length > 0) {
            const currentFileName = fileInput.files[0].name;
            fileName.textContent = '✓ ' + currentFileName;
            sessionStorage.setItem('uploadedFileName', currentFileName);
        }
        ...
        fileInput.addEventListener('change', function() {
            if (this.files.length > 0) {
                ...
                // Save to sessionStorage for browser back navigation
                sessionStorage.setItem('uploadedFileName', selectedFileName);
            }
        });
        ...
        uploadArea.addEventListener('drop', (e) => {
            ...
            if (e.dataTransfer.files.length > 0) {
                ...
                fileName.textContent = '✓ ' + droppedFileName;
                // Save to sessionStorage for browser back navigation
                sessionStorage.setItem('uploadedFileName', droppedFileName);
            }
        });
